...
Due: [Tuesday, March 21, 2023]{.orange_due}.
...
a. Interpret the results of the *t*-test. What does it mean? What is the number in the brackets?
b. "CI" means confidence interval. 
he CI has a minimum (leftmost) and maximum (rightmost) value.
...
3. Let's see what happens when we generate different samples of A and B with the **same** underlying statistics--the same $n$, same mean ($\mu$), same standard deviation ($\sigma$), same effect size ($d$), and same criterion ($\alpha$) or false positive rate.

a. Press the *Regenerate* button. What happens to the histogram? Look in the grey `t-test` box. What happens to the *t*-test, and the mean values for A, B, the difference between A and B (B-A), and the CI?
b. Press the *Regenerate* button a couple of times until `Sig?` changes from FALSE to TRUE or TRUE to FALSE? This is a form of *p*-hacking? Explain how. Remember the specific data we're analyzing are being regenerated each time we press the button; what's not changing is the sample size, standard deviation, and the effect size (difference between B and A). 

4. Having $n=75$ is a pretty large sample for many types of research in psychology, so let's see what having smaller samples does to our t-test and to our power. 
...
c. What happened to power? Does this mean we are more likely or less likely to detect a difference between A and B than before? Why?
...
a. Explore the effect of changing some other parameter on the results, for example, criterion/alpha ($\alpha$), the standard deviation for A or B ($\sigma$), or even the 'baseline' mean for B ($\mu$).
b. Report on what you changed, what you observed, and what you conclude.
